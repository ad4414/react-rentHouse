...
const Home=React.memo(()=>{
   //获取轮播图数据
    ...
    const cacheMap=new Map()
     cacheMap.set(key, useSelector(state=>state.swiper.swiperData))
     const swiperData=cacheMap.get(key)
     //获取分组数据
     cacheMap.set(key1,useSelector(state=>state.groups.groupsData))
      const groupsData=cacheMap.get(key1)
     //获取news数据
     cacheMap.set(key2,useSelector(state=>state.news.news))
    ...
    //获取点击城市
    return   <div style={{overflow:'auto',height:'980px'}}>
      {/* 搜索栏 */}
      ...
      <input type="text" onClick={()=>{
        navigate('/search')
      }} className="search-city" placeholder="搜索城市"/>
          <Button
        onClick={() => {
          navigate('/maps')
      ...
      <Carousel    className='swiper' theme='dark'>
           
            {
                ...
                swiperData.map((ele) => {
                    return (
                        ...
                        <div key={ele.id} style={{ backgroundSize: 'cover', backgroundImage: `url(http://localhost:8080${ele.imgSrc})` }}>
                        </div>
                    );
                })
            }
        ...
        <div className='groups'>
          ...
          <div className='container'>
            {groupsData.map((ele)=>{
              return  <div key={ele.id}   className={classnames('box',ele.id===id ?'highlight':'')} onClick={()=>hightLightClick(ele.id)}>
                 <div className='box2' style={{ flex: 1, textAlign: 'left' }}>
                   <Image src={`http://localhost:8080${ele.imgSrc}`} loading='lazy' className='imgGrid'/>
                 </div>
