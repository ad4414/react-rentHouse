...
/* let iconShapePoint =  new window.BMap.Symbol(window.BMap_Symbol_SHAPE_POINT, {
  ...
  fillOpacity: 0.8, // 填充透明度
    ...
    return  <div style={{ width: '100%', height: '750px' }}>
    ...
    <APILoader akay="gS19qDxhOkl8EJJXf9GfIrVjyZJ5frTB" >
      ...
      { cityPosition.map((ele)=>{
       ...
       return  <Marker key={ele.count}  onClick={showVisible} position={{lng:ele.coord.longitude ,lat:ele.coord.latitude}}    >
         {preAddress&& <Label key={ele.label} visiable={visitable} className='preLabel' 
         position={{lng:ele.coord.longitude ,lat:ele.coord.latitude}} 
         content={`${ele.label}共有房子${ele.count}套,街道名称:${preAddress && preAddress.communityName
        },社区名称:${preAddress && preAddress.streetName}`}
        style={{width:'150px', height:'63px' , whiteSpace:'pre-wrap' }}/>}
        </Marker>
