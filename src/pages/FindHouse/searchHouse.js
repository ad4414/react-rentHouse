import { NavBar,Form ,Input,Button ,Stepper,Selector,Popup,Toast,List} from "antd-mobile"
...
const SearchHouse=()=>{
 ...
 //查询房源数据
    ...
    const onFinish = (values) => {
      ...
      axios({
        url:`
        http://localhost:8080/houses?cityId=${cityId}&area=${city}&rentType=${rentType}&price=${values.price}&more=${characteristic}%2CFLOOR%7C${values.stepper}%2C${cityId}%2C${oriented}%2C${rentType}&roomType=${roomType}&oriented=${oriented}&characteristic=${characteristic}&floor=FLOOR%7C${values.stepper}&start=1&end=20`,
      }).then(result=>{
    ...
    return <div>
        <NavBar style={{backgroundColor:'aliceblue'}} onBack={back}>查找房屋</NavBar>
        <Form
        name='form'
        onFinish={onFinish}
        footer={
      ...
      >
        ...
        <Form.Item name='price' label='价格' help='将会显示满足该价格一下的房屋'>
          <Input placeholder='请输入价格' />
        </Form.Item>
        <Form.Item onClick={()=>{setVisible1(true)}}>
          {value}
        </Form.Item>
           <Popup closeOnSwipe='true'
              visible={visible}
              onMaskClick={() => {
                setVisible1(false)
              }}
    ...
    {keyCity.map((ele)=>{
       return <ul style={{fontSize:'15px'}} key={ele}>
        {ele}
        {
          cityList[ele].map(item=>{
            return  <List.Item key={item} onClick={(e)=>{
